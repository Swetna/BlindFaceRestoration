# group 11
# demo program, used for generate hq facial image
...
with torch.no_grad():
    ...
    for i, batch in enumerate(tbar):
        ...
        for i in range(2):
            ...
            hq_img1_output = cv2.cvtColor(output_img0.transpose(1, 2, 0), cv2.COLOR_BGR2RGB)
            if not os.path.exists('./results_parse/{}/iter{}'.format(result_name, i)):
                os.makedirs('./results_parse/{}/iter{}'.format(result_name, i))
            cv2.imwrite('./results_parse/{}/iter{}/{}'.format(result_name, i, name[0]), hq_img1_output)
    print("===> test done")
