...
if opt.solver == 'sgd':
    optimizer = optim.SGD(filter(lambda p: p.requires_grad, model.parameters()), lr=opt.lr, weight_decay=0.001)
    ...
    for iteration, batch in enumerate(tbar):
        ...
        if valid_iteration % opt.show_result_feq == 0:
            for i in range(opt.sum_iterations):
                ...
                if not os.path.exists('./checkpoint1/{}/retrain/tmp'.format(opt.model_name)):
                    os.makedirs('./checkpoint1/{}/retrain/tmp'.format(opt.model_name))
                cv2.imwrite('./checkpoint1/{}/retrain/tmp/SRmodule_{}_{}_iter{}_{}'.format(opt.model_name, epoch, valid_iteration,i, name[0]), output)
        train_end_time = time()
...
if __name__ == '__main__':
    ...
    if not os.path.exists('./checkpoint1/{}/retrain'.format(opt.model_name)):
        os.makedirs('./checkpoint1/{}/retrain'.format(opt.model_name))
        os.makedirs('./checkpoint1/{}/retrain/tmp'.format(opt.model_name))
    writer = SummaryWriter(log_dir='./checkpoint1/{}/retrain'.format(opt.model_name))
    # logtxt_dir = os.path.join('./checkpoint1', 'log.txt')
    losses = []
    e = []
    ...
    for epoch in range(start_epoch, opt.nEpochs + 1):
        ...
        if epoch >= opt.start_save_epoch:
            ...
            filename='./checkpoint1/{}/retrain/checkpoint_{}.pth.tar'.format(opt.model_name, epoch)
